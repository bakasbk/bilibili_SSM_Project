$(function() {
	...
	var urlArr = [ 'game-list__img1.png', 'game-list__img2.png',
			'game-list__img3.png', 'game-list__img4.png',
			'game-list__img5.png', 'game-list__img6.png', 'game-list__img7.png' ];

	$game_item.hover(function() {
		$gameImg.attr('style', 'background-image: url(images/cont/'
				+ urlArr[$(this).index()] + ')');
	}, function() {
		$gameImg.attr('style', '');
	});
	...
	var $sideBar_btn = $('.sideBar-list').find('a').not(
			$('.sideBar-list').find('a').last());
			...
			function() {
				$mainCont_item
						.each(function(i) {
							...
							if ($(window).scrollTop() > $mainCont_item.eq(i)
									.offset().top - 300) {
								$sideBar_btn.eq(i).addClass('cur').siblings()
										.removeClass('cur');
							}
	...
	$sideBar.css({
		left : widthChanged($(window).width())
	});
	...
	$.fn.slider = function(obj) {
		...
		function sliderChange(curIndex) {
			if (curIndex === slider_iNum) {
				...
				$slider_item.eq(slider_iNum).animate({
					left : -slider_w
				});
				$slider_item.eq(curIndex).css({
					left : slider_w
				});
			} else if (curIndex < slider_iNum) {
				$slider_item.eq(slider_iNum).animate({
			...
			$slider_item.eq(curIndex).animate({
				left : 0
			});
		}
		;
